...
# Example usage
class_names = load_class_names(r'Yolov3\coco.names')  # Change the file path accordingly
...
video_path=r"Video dataset\PF_EdgeCrossing_TrainArrival_1.avi"
...
model=YOLO(r"Training_Data_CenterTrack\CenterTrack_detect.pt")
model.to('cuda')
results=model(frame)
for output in results:
        parameter=output.boxes
...
while True:
    ...
    for output in results:
        ...
        for box in parameter:
            ...
            class_detect=class_names[class_detect]
            if direction=="R":
                if confidence>0.3 and class_detect == "train" and in_vision(x,y+h):
                    cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0,225), 2)
                    cv2.circle(frame,(x,y+h),10,(225,0,0),2)
                    break
                else:
                    if confidence>0.5 and class_detect == "person" and in_vision(center_x,y+h) and is_crossing_line(edge_x_bottom,edge_y_bottom,edge_x_top,edge_y_top,center_x,y+h):
                        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0,225), 2)
                ...
                if confidence>0.3 and class_detect == "train" and in_vision(x+w,y+h):
                    cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0,225), 2)
                    cv2.circle(frame,(x+w,y+h),10,(225,0,0),2)
                    break
                else:
                    if confidence>0.5 and class_detect == "person" and in_vision(center_x,y+h) and is_crossing_line(edge_x_top,edge_y_top,edge_x_bottom,edge_y_bottom,center_x,y+h):
                        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0,225), 2)
                        cv2.putText(frame, "!!!ON THE TRACK!!!", (x, y-10), font, 1, (0, 0,225), 2)
